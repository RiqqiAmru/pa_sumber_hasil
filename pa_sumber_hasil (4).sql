...
--
-- Host: 127.0.0.1
-- Generation Time: Jul 03, 2022 at 09:51 AM
...
INSERT INTO `barang` (`id_barang`, `jenis_barang`, `berat`, `baku_harga`) VALUES
...
--
-- Table structure for table `biodata_perusahaan`
--

CREATE TABLE `biodata_perusahaan` (
...
--
-- Dumping data for table `biodata_perusahaan`
--

INSERT INTO `biodata_perusahaan` (`nm_perusahaan`, `almt_perusahaan`, `telp`) VALUES
('Perusahaan Pengangkutan Uum (Express) P.A Sumber Hasil ', 'Jl. Rajawali Selatan 7 Pekalongan', '(0285) 421637');
...
INSERT INTO `cabang_perusahaan` (`cabang`, `alamat`, `telp`) VALUES
('Semarang', 'Jl. Sidorejo No.128', '(024) 3545664'),
('Jakarta', 'Hayam Wuruk Plaza II Lantai 1 - B 1166 Jl. Hayam Wuruk', '(021) 6496326'),
('Surabaya', 'Jl. Kalimas Udik I / 27', '(031) 99221962');
...
--
-- Stand-in structure for view `pelunasan`
-- (See below for the actual view)
--
CREATE TABLE `pelunasan` (
`tgl_sa` date
,`no_sa` char(7)
,`nm_pengirim` varchar(25)
,`total_bayar` int(11)
...
--
-- Stand-in structure for view `pengeluaran_kas`
-- (See below for the actual view)
--
...
('B094619', 'P-015', 'IBU LILY', 'BJM', 195000, 'lunas', 1800, 'AD-001', 2174981, '2022-06-25'),
...
('B094627', 'P-021', 'MAS JEMMY', 'KALSES EXP PELAYARAN SETIA', 200000, 'lunas', 1000, 'AD-001', 2174981, '2022-06-25'),
...
--
-- Stand-in structure for view `urutan`
-- (See below for the actual view)
--
CREATE TABLE `urutan` (
`tgl_sa` date
...
,`baku_harga` int(11)
,`bayar` int(11)
,`ketr_bayar` enum('lunas','tagihan','surabaya')
);

-- --------------------------------------------------------

--
-- Stand-in structure for view `urutan_dk`
-- (See below for the actual view)
--
CREATE TABLE `urutan_dk` (
`no_dk` int(7)
,`no_sa` char(7)
,`nm_pengirim` varchar(25)
,`alamat` varchar(30)
...
--
-- Structure for view `pelunasan`
--
DROP TABLE IF EXISTS `pelunasan`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `pelunasan`  AS SELECT `trans_pengiriman`.`tgl_sa` AS `tgl_sa`, `trans_pengiriman`.`no_sa` AS `no_sa`, `pengirim`.`nm_pengirim` AS `nm_pengirim`, `trans_pengiriman`.`total_bayar` AS `total_bayar`, `pegawai`.`nm_pegawai` AS `nm_pegawai` FROM ((`trans_pengiriman` join `pegawai`) join `pengirim`) WHERE `trans_pengiriman`.`id_pengirim` = `pengirim`.`id_pengirim` AND `trans_pengiriman`.`id_pegawai` = `pegawai`.`id_pegawai` AND `trans_pengiriman`.`ketr_bayar` in (select `trans_pengiriman`.`ketr_bayar` from `trans_pengiriman` where `trans_pengiriman`.`ketr_bayar` = 'lunas') ;

-- --------------------------------------------------------

--
-- Structure for view `pengeluaran_kas`
--
DROP TABLE IF EXISTS `pengeluaran_kas`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `pengeluaran_kas`  AS SELECT `trans_pengiriman`.`tgl_sa` AS `tgl_sa`, `pengirim`.`nm_pengirim` AS `nm_pengirim`, `detail_pengiriman`.`jml_barang` AS `jml_barang`, `ongkos_kuli`.`ongkos` AS `ongkos`, `trans_pengiriman`.`total_ongkos_kuli` AS `total` FROM (((`trans_pengiriman` join `pengirim`) join `detail_pengiriman`) join `ongkos_kuli`) WHERE `trans_pengiriman`.`no_sa` = `detail_pengiriman`.`no_sa` AND `detail_pengiriman`.`id_ongkos` = `ongkos_kuli`.`id_ongkos` AND `trans_pengiriman`.`id_pengirim` = `pengirim`.`id_pengirim` ;

-- --------------------------------------------------------

--
-- Structure for view `urutan`
--
DROP TABLE IF EXISTS `urutan`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `urutan`  AS SELECT `trans_pengiriman`.`tgl_sa` AS `tgl_sa`, `trans_pengiriman`.`no_sa` AS `no_sa`, `pengirim`.`nm_pengirim` AS `nm_pengirim`, `trans_pengiriman`.`nm_penerima` AS `nm_penerima`, `detail_pengiriman`.`jml_barang` AS `jml_barang`, `barang`.`jenis_barang` AS `jenis_barang`, `detail_pengiriman`.`nm_barang` AS `nm_barang`, `detail_pengiriman`.`total_berat` AS `total_berat`, `barang`.`baku_harga` AS `baku_harga`, `detail_pengiriman`.`bayar` AS `bayar`, `trans_pengiriman`.`ketr_bayar` AS `ketr_bayar` FROM (((`trans_pengiriman` join `pengirim`) join `detail_pengiriman`) join `barang`) WHERE `trans_pengiriman`.`no_sa` = `detail_pengiriman`.`no_sa` AND `trans_pengiriman`.`id_pengirim` = `pengirim`.`id_pengirim` AND `detail_pengiriman`.`id_barang` = `barang`.`id_barang` ;

-- --------------------------------------------------------

--
-- Structure for view `urutan_dk`
--
DROP TABLE IF EXISTS `urutan_dk`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `urutan_dk`  AS SELECT `trans_pengiriman`.`no_dk` AS `no_dk`, `urutan`.`no_sa` AS `no_sa`, `urutan`.`nm_pengirim` AS `nm_pengirim`, `pengirim`.`alamat` AS `alamat`, `urutan`.`nm_penerima` AS `nm_penerima`, `trans_pengiriman`.`alamat_penerima` AS `alamat_penerima`, `urutan`.`jml_barang` AS `jml_barang`, `urutan`.`jenis_barang` AS `jenis_barang`, `urutan`.`nm_barang` AS `nm_barang`, `urutan`.`total_berat` AS `total_berat`, `urutan`.`ketr_bayar` AS `ketr_bayar`, `urutan`.`bayar` AS `bayar` FROM ((`urutan` join `trans_pengiriman`) join `pengirim`) WHERE `urutan`.`no_sa` = `trans_pengiriman`.`no_sa` AND `trans_pengiriman`.`id_pengirim` = `pengirim`.`id_pengirim` ;
